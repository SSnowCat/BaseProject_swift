...
//        guard let possibleImage = info[.originalImage] as? UIImage else {
//            fatalError("Expected a dictionary containing an image, but was provided the following: \(info)")
//        }
        
        guard let possibleImage = info[UIImagePickerController.InfoKey.editedImage.rawValue] else {
             fatalError("Expected a dictionary containing an image, but was provided the following: \(info)")
        }
    
        newImage = (possibleImage as! UIImage)
        if let possibleImage = info[UIImagePickerController.InfoKey.originalImage.rawValue] {
            newImage = (possibleImage as! UIImage)
        }
